package com.api.churchlandmgtapi.services.auth;
...
public class AuthService {
	...
	public String Requestpasswordreset(String jsonReq) throws Exception {
        if (con == null) {
            throw new Exception("Database connection is not established");
        }
        String result = null;
        String SQL = "SELECT * FROM security.request_password_reset(?)";
		Connection conn = con;
		try (PreparedStatement pstmt = conn.prepareStatement(SQL)) {
			pstmt.setString(1, jsonReq);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				result = rs.getString("request_password_reset");
			}
			rs.close();
		} catch (SQLException e) {
			// Print Errors in console.
	...
	public String Verifypasswordresetotp(String jsonReq) throws Exception {
        if (con == null) {
            throw new Exception("Database connection is not established");
        }
        String result = null;
        String SQL = "SELECT * FROM security.verify_password_reset_otp(?)";
		Connection conn = con;
		try (PreparedStatement pstmt = conn.prepareStatement(SQL)) {
			pstmt.setString(1, jsonReq);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				result = rs.getString("verify_password_reset_otp");
			}
			rs.close();
		} catch (SQLException e) {
			// Print Errors in console.
